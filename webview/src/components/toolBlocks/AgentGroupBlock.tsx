...
const AgentGroupBlock = memo(function AgentGroupBlock({
  ...
  useEffect(() => {
    ...
    // 两侧的短路条件是互补的，都要保留：
    // history —— 本仓库加的，历史已拉到就别再轮询（省掉一次无用往返）
    // isError —— 上游 v0.4.8 加的，失败的任务不该继续轮询（否则永远轮下去）
    if (!expanded || !currentSessionId || !toolId || isCompleted || isError || history) {
  ...
  }, [agentId, currentSessionId, summary, expanded, history, isCompleted, isError, toolId]);
